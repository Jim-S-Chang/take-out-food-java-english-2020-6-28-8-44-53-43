...
public class App {
    ...
    public String bestCharge(List<String> inputs) {
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            inputIDs.add(split[0]);
            numberOfItems.add(Integer.parseInt(split[split.length - 1]));
        }
        ...
        if(totalPrice <= firstPromotion && totalPrice <= secondPromotion){ // 没有优惠可以使用
            ...
            // 生成使用第二个优惠的商品的名字的字符串
            String saleItem = new String();
            for (int i = 0; i < salesItemName.size(); i++) {
                saleItem += salesItemName.get(i);
                if (i != salesItemName.size() - 1) {
                     saleItem += ",";
                }
            }
